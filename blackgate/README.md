...
third-party imports, and each runnable on its own. Six modules, **501 of the
repository's 1,591 tests**, and one real printed run per module that takes a
second to reproduce.
...
python3 tests/mutation_harness.py                 # all 120; runtime varies
```

**Thirty eight mutations across these six modules, every one caught, 196 test
...
xychart-beta
    ...
    bar [1, 10, 1, 3, 1, 3, 10, 4, 19, 5, 7, 3, 7, 3, 19, 5, 3, 4, 2, 1, 10, 1, 26, 3, 1, 5, 7, 7, 4, 1, 2, 5, 6, 1, 1, 2, 1, 2]
```
...
eight sum to **196**. A run whose test count differs from the baseline is reported as
...
| AT3 | `attestation.py` | the nonce is no longer single use | 19 |
...
| AU3 | `audit_chain.py` | a detail field is hashed without being redacted | 19 |
...
| AU6 | `audit_chain.py` | a secret under a quoted field name reaches the hashed bytes | 4 |
