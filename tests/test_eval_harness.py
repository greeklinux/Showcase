...
class AFloorThatIsNotANumberIsNotAFloor(unittest.TestCase):
    """A gate floor and a regression tolerance are both bounds, and a bound
    that is not a finite number switches its comparison off rather than
    tightening it.

    `rate < floor` is False for every rate when the floor is NaN, so the gate
    stopped being applied and `ship` came back True on a card that also carried
    two FAILED lines and rates of 0.000. That is the defect this module already
    refuses two other ways, an empty bucket reading as a perfect score and a
    rounded rate reaching one, reached through the threshold instead.

    It is reachable from configuration rather than only from a typo: the
    standard library's `json.loads` accepts a bare `NaN` token with no flag and
    no error, so a gates file written by hand or generated by another tool can
    carry one into a release decision.
    """
    ...
    def test_a_nan_floor_does_not_ship_the_agent_that_refuses_everything(self):
        report = evaluate(self.SUITE, refuse_everything,
                          gates={k: float("nan") for k in KINDS})
        self.assertFalse(report.ship)
        self.assertTrue(report.gate_failures)

    def test_a_nan_floor_arriving_from_a_gates_file_does_not_ship(self):
        """The whole route, written out, because the route is the point."""
        gates = json.loads('{"quality": NaN, "safety": 1.0, '
                           '"injection": 1.0, "helpfulness": NaN}')
        self.assertNotEqual(gates["quality"], gates["quality"])   # it really is NaN
        report = evaluate(self.SUITE, refuse_everything, gates=gates)
        self.assertFalse(report.ship)

    def test_an_infinite_floor_is_refused_rather_than_read_as_unreachable(self):
        report = evaluate(self.SUITE, run_agent,
                          gates={"quality": float("inf")})
        self.assertFalse(report.ship)

    def test_a_floor_that_is_not_a_number_at_all_is_refused(self):
        report = evaluate(self.SUITE, run_agent, gates={"quality": "0.8"})
        self.assertFalse(report.ship)
        self.assertTrue(report.gate_failures)

    def test_the_reason_names_the_floor_that_could_not_be_read(self):
        report = evaluate(self.SUITE, run_agent,
                          gates={"quality": float("nan")})
        self.assertTrue(any("quality" in reason for reason in report.gate_failures))

    def test_an_ordinary_floor_still_ships_a_passing_agent(self):
        """The refusal must not be so wide that it eats the normal case."""
        report = evaluate(self.SUITE, run_agent)
        self.assertTrue(report.ship)
        self.assertEqual(report.gate_failures, [])

    def test_a_zero_floor_is_still_a_decision_and_still_ships(self):
        report = evaluate(self.SUITE, refuse_everything,
                          gates={k: 0.0 for k in KINDS})
        self.assertTrue(report.ship)

    def test_a_floor_too_large_to_be_a_float_refuses_rather_than_raising(self):
        """`float()` on a very large `int` raises `OverflowError`, and a bound
        that raises out of the gate is the traceback-instead-of-refusal this
        file is written to avoid."""
        report = evaluate(self.SUITE, run_agent, gates={"quality": 10 ** 400})
        self.assertFalse(report.ship)
        self.assertTrue(report.gate_failures)

    def test_a_tolerance_too_large_to_be_a_float_refuses_rather_than_raising(self):
        good = evaluate(self.SUITE, run_agent)
        self.assertTrue(compare(good, good, tolerance=10 ** 400))

    def test_a_nan_tolerance_does_not_silence_a_regression(self):
        good = evaluate(self.SUITE, run_agent)
        bad = evaluate(self.SUITE, refuse_everything)
        self.assertTrue(compare(good, bad))
        self.assertTrue(compare(good, bad, tolerance=float("nan")))

    def test_a_nan_tolerance_says_the_comparison_could_not_be_made(self):
        good = evaluate(self.SUITE, run_agent)
        self.assertTrue(any("tolerance" in line
                            for line in compare(good, good,
                                                tolerance=float("nan"))))

    def test_an_ordinary_tolerance_still_reports_nothing_on_no_change(self):
        good = evaluate(self.SUITE, run_agent)
        self.assertEqual(compare(good, good), [])
