...
def probe_bounded_attestation():
    ...
    for index in range(1000):
        store.consume("n%d" % index, index, expires_at=index)
    dropped = store.evict_before(900)
    _assert(dropped == 900 and sum(row[0] is not None for row in store.journal) == 100,
            "the spent nonce store is not bounded by signed expiry")
