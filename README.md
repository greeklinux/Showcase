...
| [`tests/`](tests/) | One test file per module, named as sentences. Standard library `unittest`, plus a mutation harness that breaks the code on purpose to prove the suite can fail. | 1,435 tests |
...
Six modules contribute **433 of the suite's 1,435 tests**, covering the failure modes described below.
...
declined, and the twenty nine mutations
...
### Where the 1,435 tests are

```mermaid
sankey-beta

the suite,ai_security,638
the suite,blackgate,433
...
ai_security,control_flow_audit,75
ai_security,differential_consistency,72
ai_security,provenance_algebra,66
ai_security,eval_harness,65
ai_security,mount_audit,55
ai_security,agentic_soc,44
blackgate,scope_gate,90
blackgate,attestation,74
blackgate,detection_gap,71
blackgate,audit_chain,69
blackgate,prohibitions,65
blackgate,approval_ceremony,64
polymind,adaptive_signal,61
polymind,posterior,56
polymind,calibration,42
polymind,signal_fusion,40
polymind,evidence_gate,35
polymind,devig,34
polymind,honest_states,34
polymind,method_graft,33
...
twenty four parts sum to **1,435**, which is what
...
[further down this page](#where-the-1435-tests-are-file-by-file).
...
quadrantChart
    ...
    "scope_gate": [0.554, 0.750]
    "llm_output_validator": [0.284, 0.708]
    "capability_attenuation": [0.551, 0.625]
    "control_flow_audit": [0.824, 0.625]
    "attestation": [0.406, 0.617]
    "differential_consistency": [0.566, 0.600]
    "detection_gap": [0.424, 0.592]
    "audit_chain": [0.456, 0.575]
    "provenance_algebra": [0.450, 0.550]
    "eval_harness": [0.216, 0.542]
    "prohibitions": [0.360, 0.542]
    "approval_ceremony": [0.361, 0.533]
    "adaptive_signal": [0.211, 0.508]
    "posterior": [0.229, 0.467]
    "mount_audit": [0.271, 0.458]
...
**Ninety two mutations, ninety two caught, zero survivors, 401 test
deaths.** The mutations are declared as data in
...
make test                                    # 1,435 tests, standard library unittest
...
<a id="where-the-1435-tests-are-file-by-file"></a>

<details>
<summary><b>Where the 1,435 tests are, file by file</b></summary>
...
| --- | ---: | --- | ---: |
| [`prompt_guard.py`](ai_security/prompt_guard.py) | 101 | [`approval_ceremony.py`](blackgate/approval_ceremony.py) | 64 |
| [`scope_gate.py`](blackgate/scope_gate.py) | 90 | [`adaptive_signal.py`](polymind/adaptive_signal.py) | 61 |
| [`llm_output_validator.py`](ai_security/llm_output_validator.py) | 85 | [`posterior.py`](polymind/posterior.py) | 56 |
| [`capability_attenuation.py`](ai_security/capability_attenuation.py) | 75 | [`mount_audit.py`](ai_security/mount_audit.py) | 55 |
| [`control_flow_audit.py`](ai_security/control_flow_audit.py) | 75 | [`agentic_soc.py`](ai_security/agentic_soc.py) | 44 |
...
| [`audit_chain.py`](blackgate/audit_chain.py) | 69 | [`devig.py`](polymind/devig.py) | 34 |
| [`provenance_algebra.py`](ai_security/provenance_algebra.py) | 66 | [`honest_states.py`](polymind/honest_states.py) | 34 |
| [`eval_harness.py`](ai_security/eval_harness.py) | 65 | [`method_graft.py`](polymind/method_graft.py) | 33 |
| [`prohibitions.py`](blackgate/prohibitions.py) | 65 | [`alert_deduper.py`](automation/alert_deduper.py) | 29 |

Counted by running each file on its own with `python3 -m unittest tests.<name>`
and reading the `Ran N tests` line. The parts sum to **1435**, which is what the
