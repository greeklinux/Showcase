...
| --- | --- | --- |
| [**`blackgate/`**](blackgate/) &nbsp; **BlackGate** | The safety architecture of an authorized, human-gated adversary emulation and purple team platform: two physically separated planes, a fail-closed scope gate, an approval bound to one exact ordered argument list, a four-stage two-person ceremony, the prohibition no approval reaches, a keyed audit chain with an external witness, and the scoring loop that closes the detection gap. | 6 modules, 501 tests |
...
| [`ai_security/`](ai_security/) | Defense in depth for autonomous systems: input guard, output guard, mount-surface auditor, release gate, multi-agent SOC triage, and four pathways that ask whether a control is wired to the decision at all. | 9 modules, 695 tests, 3 KQL hunts |
...
| [`tests/`](tests/) | One test file per module, named as sentences. Standard library `unittest`, plus a mutation harness that breaks the code on purpose to prove the suite can fail. | 1,591 tests |
...
Six modules contribute **501 of the suite's 1,591 tests**, covering the failure modes described below.
...
### Where the 1,591 tests are

```mermaid
sankey-beta

the suite,ai_security,695
the suite,blackgate,501
...
ai_security,control_flow_audit,98
...
twenty four parts sum to **1,591**, which is what
...
quadrantChart
    ...
    "control_flow_audit": [0.723, 0.817]
    "attestation": [0.344, 0.733]
    "differential_consistency": [0.396, 0.600]
    "detection_gap": [0.324, 0.642]
    "audit_chain": [0.387, 0.675]
    "provenance_algebra": [0.340, 0.625]
    "eval_harness": [0.157, 0.567]
    "prohibitions": [0.275, 0.592]
    "approval_ceremony": [0.306, 0.683]
    "adaptive_signal": [0.148, 0.508]
    "posterior": [0.160, 0.467]
    "mount_audit": [0.289, 0.575]
    "agentic_soc": [0.141, 0.383]
    "calibration": [0.109, 0.400]
    "signal_fusion": [0.093, 0.383]
    "evidence_gate": [0.144, 0.342]
    "devig": [0.065, 0.283]
    "honest_states": [0.123, 0.333]
    "method_graft": [0.145, 0.333]
...
make test                                    # 1,591 tests, standard library unittest
...
<details>
<summary><b>Where the 1,591 tests are, file by file</b></summary>
...
| --- | ---: | --- | ---: |
| [`prompt_guard.py`](ai_security/prompt_guard.py) | 102 | [`approval_ceremony.py`](blackgate/approval_ceremony.py) | 82 |
...
| [`capability_attenuation.py`](ai_security/capability_attenuation.py) | 80 | [`mount_audit.py`](ai_security/mount_audit.py) | 69 |
| [`control_flow_audit.py`](ai_security/control_flow_audit.py) | 98 | [`agentic_soc.py`](ai_security/agentic_soc.py) | 46 |
| [`attestation.py`](blackgate/attestation.py) | 88 | [`calibration.py`](polymind/calibration.py) | 48 |
...
| [`eval_harness.py`](ai_security/eval_harness.py) | 68 | [`method_graft.py`](polymind/method_graft.py) | 40 |
...
and reading the `Ran N tests` line. The parts sum to **1591**, which is what the
